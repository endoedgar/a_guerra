...
bool StackTrace::InitSymbols()
{
	...
	if (!ls_initialized)
	{
		...
		{
			OutputDebugStringA("Cannot initialize symbol engine");
			return false;
		}
...
{
	HANDLE hDump = ::CreateFileA(fileName, GENERIC_WRITE, FILE_SHARE_READ, 0,
...
LONG WINAPI MyExceptionFilter(EXCEPTION_POINTERS* exceptionPtrs)
{
	...
	std::stringstream ss;
	ss << "O programa não pode continuar!" << std::endl;
	ss << "Causa: " << GetExceptionString(exceptionPtrs->ExceptionRecord->ExceptionCode) << std::endl;
	ss << "Endereço: " << exceptionPtrs->ExceptionRecord->ExceptionAddress << std::endl;
	ss << "Deseja criar um dump da memória?" << std::endl;

	if(MessageBoxA(NULL, ss.str().c_str(), "Erro de execução!", MB_YESNO | MB_ICONERROR) == IDYES) {	
		...
		{
			OutputDebugStringA("*** FATAL ERROR: EXCEPTION_STACK_OVERFLOW detected!");
		}
